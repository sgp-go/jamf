 ...
 *   2. --execute（須 Windows ops 機：git + pwsh + WiX；設備無 git，見 handoff 待辦 #3）：
 *      實構建 + 註冊 + 派發。建議排程定時 dry-run 告警、人工確認後再 --execute。
 *
...
const deps = makeDefaultDeps({ certThumbprint: flags["cert-thumbprint"] });
...
if (r.dryRun) {
  console.log(
    "\n⚠️ dry-run：未構建未派發。確認無誤後加 --execute 在有 git+pwsh+WiX 的 ops 機實跑。",
  );
