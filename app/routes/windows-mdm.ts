...
async function handleEnrollmentRequest(c: Context, slug: string, groupCode?: string) {
  ...
  // 自動設備命名（PRD §5.1）：走 reconcileDeviceName 統一收斂到 namingTemplate 展開結果。
  // template 變數 {schoolCode}/{serial}/{serial4}/{udid8}。
  // enrollment 當下 device.serialNumber 通常尚未 pull（要等 agent 首次上報 backfill），含 {serial*}
  // 的 template 此時會 skip awaiting_serial（不再硬派 "0000"），由後續 agent report 的
  // reconcile（onReport）在序號到位後補派正確名稱。{udid8}-only template enroll 當下即可派。
  try {
    const enrolledDevice = await getMdmDevice(udid);
    if (enrolledDevice) {
      const decision = await reconcileDeviceName({
        tenantId: enrolledDevice.tenantId,
        deviceId: enrolledDevice.id,
      });
      console.log(
        `[Win MDM] 自動命名 reconcile udid=${udid} action=${decision.action}` +
          (decision.action === "dispatch"
            ? ` name="${decision.desiredName}"`
