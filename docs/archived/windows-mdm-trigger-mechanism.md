...
1. **`.env` WNS 凭据**：`WNS_PACKAGE_SID` + `WNS_CLIENT_SECRET` + `WNS_PFN` 三件套（見 [windows-mdm-account-setup.md](./windows-mdm-account-setup.md)）
2. **push-capable MSIX 已裝**：PFN 必須 == `WNS_PFN`，含 `IBackgroundTask` 實現 + manifest 三件套（見 [windows-mdm-msix-signing.md](./windows-mdm-msix-signing.md) 第 3 節）
