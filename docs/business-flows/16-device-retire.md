...
## 遠端重新部署（/redeploy）— 保留 PPKG 自動回管（PRD §5.1）

與退役相反：redeploy 也做 Wipe，但**保留 PPKG**，設備重置後自動重跑 OOBE、走完整佈建流程回到**同一** tenant / device_group。適用「學期末統一還原到出廠佈建狀態」或「政策層混亂，回歸出廠 + 自動重跑 enrollment hook」。設備**不**標記 unenrolled（因為它會自動回管）。
...
sequenceDiagram
    ...
    Admin->>Backend: POST /admin/.../devices/{id}/redeploy
    Backend->>MDM: 派 Wipe（Windows: doWipePersistProvisionedData / Apple: ERASE_DEVICE）
    MDM->>Device: 推送 Wipe 指令
    Backend->>Backend: 寫 audit log
    Backend-->>Admin: 200 { deviceId, wipe }

    Device->>Device: 工廠重置，**保留 PPKG**
    Device->>Device: 自動重跑 OOBE → enrollment hook
    Note over Device,Backend: 落回原 (tenant, device_group)<br/>Agent / WiFi / 初始帳號自動佈建回來
```

### 三種「重置」的區別

| 端點 | Wipe 動作 | PPKG | 分組 | 結果 |
|------|-----------|------|------|------|
| `/redeploy` | `doWipePersistProvisionedData` | **保留** | 不改 | 重置後自動回管到**同一**分組 |
| `/transfer` | `doWipePersistProvisionedData` | **保留** | **換到新 group** | 重置後自動歸入**新**分組（見 [10-device-transfer](10-device-transfer.md)） |
| `/retire` | `doWipe`（預設） | **一併抹除** | — | 徹底脫管，**不**自動回管 |

> `redeploy` 與 `transfer` 底層同一個 `doWipePersistProvisionedData`，差別只在轉校會先把 `device_group` 改成目標校；redeploy 不動分組。
