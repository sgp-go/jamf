...
```
/t/{tenantSlug}/EnrollmentServer/*     ← Windows SOAP enrollment（多租戶）
...
| Method | Path | 用途 |
|---|---|---|
| `POST` | `/admin/tenants/{tid}/enrollment/ppkg-config` | 生成 customizations.xml（含 WiFi / 本機帳號配置） |
...
你方提供一個 HTTPS POST URL + 事件類型清單，**目前由我方 ops 手動寫入**（自助註冊 API 尚未實作）。
我方回傳 `secret`（HMAC 簽名密鑰），請妥善保管——僅交付一次。

> ⚠️ **現狀**：webhook endpoint 註冊走 ops 流程，不是 API。對接初期請在啟動會議提供接收 URL，
> 我方代為配置。自助註冊 endpoint 在 backlog。

**Webhook 接入工作表**（啟動時提供給我方 ops，每個接收端點一份）：

| 欄位 | 由誰提供 | 說明 |
|---|---|---|
| `tenant_id` | 你方 | 此 endpoint 屬於哪個租戶 |
| 接收 URL | 你方 | HTTPS POST，建議帶路徑版本（如 `https://api.tw.example/cogrow/webhook/v1`） |
| 訂閱事件類型 | 你方 | 從 §8.6 清單勾選；留空＝全訂閱 |
| `secret`（HMAC 密鑰） | **我方回傳** | 我方寫入後一次性交付，你方存入密鑰管理；遺失需重新生成 |
| 啟用狀態 | 我方 | 寫入即生效 |

> **變更流程**：換 URL、改訂閱、輪換 secret 目前同樣走 ops（重發此工作表），不能自助。
> 測試→生產切換時，建議**新建一個生產 endpoint**而非改測試 endpoint，驗證無誤後再停用測試的。
> 自助 CRUD（`POST/PATCH/DELETE .../webhook-endpoints`）在 backlog。
