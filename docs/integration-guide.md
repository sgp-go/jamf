...
| Method | Path | 用途 |
|---|---|---|
| `POST` | `/admin/tenants/{tid}/devices/{did}/transfer` | 跨校轉移（改分組 + 觸發 Wipe，⚠️ **需 IT 現場插 USB 新 PPKG**）|
| `POST` | `/admin/tenants/{tid}/devices/{did}/redeploy` | 遠端重新部署（PRD §5.1）：Wipe 保留 PPKG，⚠️ **需 IT 現場再跑 `Install-ProvisioningPackage`**（doWipePersistProvisionedData 不會自動重跑 PPKG，真機驗證 2026-07-03）|
| `POST` | `/admin/tenants/{tid}/devices/{did}/soft-wipe` | **軟 Wipe（推薦畢業換人場景）**：Agent 清乾淨學生痕跡，保 Agent + MDM enrollment，秒級完成，零 IT 介入 |
| `POST` | `/admin/tenants/{tid}/devices/{did}/cancel-soft-wipe` | 取消未執行的 soft-wipe（緊急止血，窗口只有幾秒） |
| `POST` | `/admin/tenants/{tid}/devices/{did}/retire` | 設備退役（doWipe 徹底 + 標記 unenrolled，不回管） |
| `POST` | `/admin/tenants/{tid}/devices/{did}/install-agent` | 一鍵派 Agent MSI + 注入配置（Windows） |
| `POST` | `/admin/tenants/{tid}/devices/{did}/apps/{appId}/install` | 派發任意 MSI App 安裝到設備（不簽 agent_token；秒級 WNS 喚醒） |
| `POST` | `/admin/tenants/{tid}/devices/{did}/apps/{appId}/uninstall` | 派發 MSI App 卸載命令（EDA-CSP Delete verb 觸發 msiexec /x） |
...
| `POST` | `/admin/tenants/{tid}/devices/{did}/laps-rotate` | 手動觸發 LAPS 密碼輪換（目標帳號從 tenant config 讀，預設 ITAdmin） |
| `POST` | `/admin/tenants/{tid}/devices/{did}/user-password/reset` | **通用密碼重設**（含學生）：管理員指定 targetAccount + mode（explicit/random）+ 可選強制首登改密。復用 LAPS 通道 |
| `GET` | `/admin/tenants/{tid}/devices/{did}/user-password/{targetAccount}` | 查任意帳號最新密碼（跨 admin/student/other） |
| `GET` | `/admin/tenants/{tid}/devices/{did}/bitlocker-recovery` | 查詢 BitLocker Recovery Password（解密明文，寫 audit log） |

**Admin 帳號名可配**：`self_mdm_configs.admin_account_name`（VARCHAR(64) NOT NULL DEFAULT `'ITAdmin'`）—— Win11 內建 Administrator 預設禁用不可用，PPKG 建的 ITAdmin 才是日常 admin。若 PPKG 建的帳號名不同，`PATCH /admin/tenants/{tid}/mdm-config` 改 `adminAccountName`。

**同步延遲注意**：Agent v1.4.0.22 或更早，改密後 backend status 停 pending 最長 24h（等 daily report slot）。v1.4.0.23+ 加了「主動 POST /agent/checkin + registry 兜底」三級 confirm，秒級 confirmed。詳見 `docs/windows-deployment/laps-password-management.md §7`。
...
**策略預設**（高層封裝，建 profile 供批量指派用）：

| Method | Path | 用途 |
|---|---|---|
| `POST` | `/admin/tenants/{tid}/profile-presets/blocked-sites` | 網站黑名單 profile |
| `POST` | `/admin/tenants/{tid}/profile-presets/defender` | Defender 強制啟用 profile |
| `POST` | `/admin/tenants/{tid}/profile-presets/update-policy` | Windows Update 策略 profile |

**設備直推**（POST /admin/tenants/{tid}/devices/{did}/... ，不建 profile 直派命令）：

| Path | 用途 | PRD |
|---|---|---|
| `/push-wifi` / `/remove-wifi` | WiFi 連線 | §5.2 |
| `/push-wallpaper` | 桌布 / 鎖屏圖 | §5.2 |
| `/push-password-policy` | 密碼政策 | §5.4 |
| `/push-usb-policy` | USB 儲存禁用（讀 / 寫） | §5.4 |
| `/push-app-restriction` | AppLocker 白/黑名單 | §3.9 |
| `/push-vpn` / `/remove-vpn` | VPN 連線 | §5.2 |
| `/push-camera-policy` | 相機啟用 / 禁用 | §5.4 |
| `/push-firewall-policy` | 防火牆強制啟用 | §5.4 |
| `/push-device-install-policy` | DeviceInstallation 全類別禁用（Setup Class GUID / Hardware ID / removable-flag） | §5.4 進階 |
| `/clear-device-install-policy` | 清除 DeviceInstallation 政策 | §5.4 |
| `/rename` | 手動設備命名（單台）。enroll 後自動命名改設 `PATCH /mdm-config` 的 `namingTemplate`（agent 上報後 reconcile 自動套用）。走 agent 信箱非 Accounts CSP（workgroup 設備 406） | §5.1 |
| `/push-settings-restriction` | Settings 頁面隱藏 | §5.2 |
| `/push-lost-mode` / `/remove-lost-mode` | 遺失模式（LegalNotice + Registry 信箱） | §5.2 |
| `/push-defender` | Defender 強制啟用（`all=true` 全開 / `custom` 覆蓋） | §4.1.2 |
| `/query-defender-health` | 查 Defender 健康狀態（16 個節點可選） | §4.1.2 |
| `/push-blocked-sites` | 網站黑名單（**Edge URLBlocklist ADMX + IE Zone 4 兩層 batch**） | §4.1.1 |
| `/clear-blocked-sites` | 清除黑名單 | §4.1.1 |
| `/push-edge-browser-signin` | Edge 帳號登入政策（mode 0=Disable / 1=Enable / 2=Force） | §4.1.1 配套 |
| `/clear-edge-browser-signin` | 清除 BrowserSignin | §4.1.1 |
| `/push-update-policy` | Windows Update 完整策略（Schedule / Defer / Pause） | §5.6 |
| `/trigger-os-update` | 立即觸發 OS 更新（強制自動 + 短排程） | §5.6 |
| `/query-update-status` | 查更新狀態（installable / installed / pendingReboot） | §5.6 |

⭐ **`/push-blocked-sites` 三個關鍵坑**（2026-07-03 真機驗證）：

1. **必須用 bare host 語法**：`hosts:["tiktok.com","facebook.com"]` 直接傳，**別**手動包成 `*://tiktok.com/*` 或 `https://tiktok.com/`——Chromium URL filter 的 `/*` 是 literal path 反而不匹配。Bare host 自動 match host + 所有 subdomain
2. **IE Zone 4 對 Edge Chromium URL 加載無效**：端點內部一次派 3 條（Edge ADMX install + Edge URLBlocklist + IE Zone），IE Zone 保留是給 IE Mode / ActiveX 場景。真正封 Edge 靠 Edge URLBlocklist
3. **Edge 已開時 policy 冷讀**：Chromium dynamic policy refresh 對 URLBlocklist 常有 15-90min 延遲，改 policy 後**必須 kill 全部 msedge.exe**（或 15-90min 等）才穩定生效。搭配 `/push-edge-browser-signin` mode=0 防學生登 MS 個人帳號繞過（MS account signed-in profile 對 URLBlocklist 免疫，by design）

### 5.8 Agent 上報（Tenant，iOS + Windows 共用）

| Method | Path | 用途 |
|---|---|---|
| `POST` | `/tenants/{tid}/agent/enroll` | **Intune 共存**：設備自助換 per-device token（帶共享密鑰 + 序號，**無 Bearer**）；見 §7.4 |
...
| `POST` | `/tenants/{tid}/agent/gps` | 上報 GPS 位置（Lost Mode 高頻 / 平時每日）|
| `POST` | `/tenants/{tid}/agent/winget-result` | Agent 上報 winget 執行結果 |
| `POST` | `/tenants/{tid}/agent/soft-wipe-result` | Agent 上報 soft-wipe 執行結果（觸發 `device.soft_wiped` webhook） |
...
| `POST` | `/admin/tenants/{tid}/agent-enrollment-secret` | **Intune 共存**：生成 / 輪換 Agent 自助註冊共享密鑰（明文僅回一次） |
| `DELETE` | `/admin/tenants/{tid}/agent-enrollment-secret` | 撤銷自助註冊密鑰（關閉自助註冊；已簽發 token 不受影響） |
...
### 5.14 Geofence 地理圍欄（Admin）

PRD §6 Future 落地。學校場景「設備離開校園自動觸發警報 / 派 Lost Mode」的後端閉環：admin 定義多邊形圍欄 → 關聯設備 → Agent GPS 上報時後端 point-in-polygon 檢測 → transition 發生 webhook。

| Method | Path | 用途 |
|---|---|---|
| `POST` | `/admin/tenants/{tid}/geofences` | 建 geofence（polygon 至少 3 個 `{lat, lng}` 頂點） |
| `GET` | `/admin/tenants/{tid}/geofences` | 列 geofence |
| `GET` | `/admin/tenants/{tid}/geofences/{gid}` | 詳情 |
| `PATCH` | `/admin/tenants/{tid}/geofences/{gid}` | 更新（部分欄位；polygon 提供則整份覆蓋） |
| `DELETE` | `/admin/tenants/{tid}/geofences/{gid}` | 刪除（**級聯**清 assignments + states） |
| `POST` | `/admin/tenants/{tid}/devices/{did}/geofences/{gid}/assign` | 設備關聯 geofence（幂等） |
| `POST` | `/admin/tenants/{tid}/devices/{did}/geofences/{gid}/unassign` | 解除關聯（同步清 state 避免遺留） |
| `GET` | `/admin/tenants/{tid}/devices/{did}/geofences` | 查設備當前所有 geofence in/out 狀態 |

**Webhook 事件**（見 §8.6）：
- `device.geofence_enter` — 設備進入圍欄
- `device.geofence_exit` — 設備離開圍欄

**設計要點**：

- **首次落表不發 webhook**：assign 後首次 GPS 上報只落基線 state（inside 或 outside），不觸發 enter/exit 事件（避免大量設備 assign 時噪音）；只有真的 transition 才發
- **多對多 assignment**：一台設備可關聯多個 geofence（跨校區 / 多層圍欄）；一個 geofence 可覆蓋多台
- **依賴 Agent GPS**：Agent 未上報過位置的設備，state 表無 row，`GET .../geofences` 該 geofence 的 `status` 欄位為 `null`
- **polygon 語意**：頂點順時針 / 逆時針皆可，首尾不必相同（算法自動閉合）；至少 3 個點；WGS84 度數
...
```bash
# 建校園圍欄
POST /admin/tenants/{tid}/geofences
{
  "name": "光復國小校園",
  "polygon": [
    {"lat": 25.045, "lng": 121.517},
    {"lat": 25.045, "lng": 121.520},
    {"lat": 25.048, "lng": 121.520},
    {"lat": 25.048, "lng": 121.517}
  ],
  "isActive": true
}
# → 201 { id: "..." }
...
# 把一台設備關聯進去
POST /admin/tenants/{tid}/devices/{did}/geofences/{gid}/assign
# → 204

# 收到 device.geofence_exit webhook 後可自動觸發 Lost Mode：
POST /admin/tenants/{tid}/devices/{did}/push-lost-mode
{"message":"設備已離校，請聯絡...","phone":"02-1234-5678"}
```
...
### 7.4 Intune 共存（遙測 only）

存量設備仍歸 **Microsoft Intune** 管、暫不遷移時，可用 Intune 下發只帶「共享密鑰」的 Agent MSI，讓設備自助換 token 接入**遙測層**（使用時長 / 設備資訊 / 已裝軟體 / 電量 / 網路）。

> ⚠️ **能力邊界**：此路徑只換**遙測**。管理面（遠端鎖定 / wipe / LAPS / BitLocker / CSP 策略）仍歸 Intune —— 一台 Windows 的 OMA-DM 通道只能歸一個 MDM。要全套管理須脫 Intune、走自建 PPKG/OMA-DM 納管。這是**共存 / 過渡**方案。

```
1. 生成共享密鑰
   POST /admin/tenants/{tid}/agent-enrollment-secret → enrollmentSecret（明文僅回一次）
2. Intune 打包 Agent MSI（全租戶通用同一份），安裝命令行帶：
   msiexec /qn API_ENDPOINT=https://<host>/api/v1 TENANT_ID=<tid> ENROLLMENT_SECRET=<密鑰>
   （不帶 DEVICE_ID / AGENT_TOKEN；同一條命令行派給所有設備）
3. Agent 首啟：無 token 但有密鑰 → POST /tenants/{tid}/agent/enroll（序號 + 密鑰）
   → 後端建 windows/agent_only/self_mdm_managed=false 設備 + 簽 token
   → Agent 寫回 registry + 刪密鑰
4. 之後 checkin / reports / usage 走 Bearer，與 install-agent 設備完全同路徑
```

- 設備標記 `platform=windows` / `enrollment_type=agent_only` / `self_mdm_managed=false`（僅遙測，不宣稱管理面）。
- 密鑰**可輪換 / 可撤銷**（見 §5.9）；per-device token 一旦簽發即與密鑰解耦。
- 詳細操作 + Intune 打包步驟：[windows-deployment/intune-coexistence-agent-enroll.md](windows-deployment/intune-coexistence-agent-enroll.md)；流程圖：[business-flows/23-intune-coexistence-agent-enroll.md](business-flows/23-intune-coexistence-agent-enroll.md)。
...
| 地理圍欄 | `device.geofence_enter` / `device.geofence_exit` |
| 軟 Wipe | `device.soft_wipe_started` / `device.soft_wiped` / `device.soft_wipe_failed` |
...
| Agent | `agent.installed` / `reported` / `checkin` / `usage_reported` / `usage_anomaly` / `gps_reported` |
...
| 2026-07-03 | **v0.6** | **PRD §4/§5/§6 新功能一併補完**：§5.5 新增 `/redeploy`（PRD §5.1 遠端重新部署，wipe 保留 PPKG 不改分組）；§5.7 新增 11 個設備直推端點（push-defender / query-defender-health / push-blocked-sites / clear-blocked-sites / push-edge-browser-signin / clear-edge-browser-signin / push-update-policy / trigger-os-update / query-update-status / push-device-install-policy / clear-device-install-policy）；⭐ 網站黑名單踩坑總結（bare host 語法、Edge Chromium URL filter `/*` 陷阱、Dynamic Policy Refresh 冷讀）；新增 §5.14 Geofence 地理圍欄 8 端點（PRD §6，含 point-in-polygon 後端閉環 + `device.geofence_enter/exit` webhook）；§8.6 事件清單同步 |
| 2026-07-03 | **v0.7** | **Soft Wipe（畢業換人零 IT 介入）**：§5.5 新增 `/soft-wipe` + `/cancel-soft-wipe` 端點；Agent SYSTEM 權限清乾淨學生痕跡（user profile / 用戶自裝 App / 瀏覽器數據 / Recycle Bin / Temp）保 Agent + MDM；§5.8 新增 `/agent/soft-wipe-result` 端點；§8.6 事件加 `device.soft_wipe_started` / `.soft_wiped` / `.soft_wipe_failed`；⭐ 真機驗證 `doWipePersistProvisionedData` **不會自動重跑 PPKG**（memory 舊判斷收回），故 `/transfer` `/redeploy` 明確標註「需 IT 現場介入」 |
...
| 2026-07-13 | **v0.9** | **Intune 共存（遙測 only）Agent 自助註冊**：§5.8 新增 `/agent/enroll`（設備帶 tenant 級共享密鑰自助換 per-device token，無 Bearer）；§5.9 新增 `agent-enrollment-secret` POST/DELETE（生成 / 輪換 / 撤銷共享密鑰）；新增 §7.4 + `windows-deployment/intune-coexistence-agent-enroll.md` + `business-flows/23`；設備標記 `agent_only` / `self_mdm_managed=false`；⭐ 真機驗證通過（Agent 1.4.0.99 Intune-mode 自註冊 + 6 reports + 11 usage 全綠） |
