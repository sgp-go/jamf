...
| 2026-07-03 | **v0.6** | **PRD §4/§5/§6 新功能一併補完**：§5.7 新增 9 個設備直推端點（push-defender / query-defender-health / push-blocked-sites / clear-blocked-sites / push-edge-browser-signin / clear-edge-browser-signin / push-update-policy / trigger-os-update / query-update-status）；⭐ 網站黑名單踩坑總結（bare host 語法、Edge Chromium URL filter `/*` 陷阱、Dynamic Policy Refresh 冷讀）；新增 §5.14 Geofence 地理圍欄 8 端點（PRD §6，含 point-in-polygon 後端閉環 + `device.geofence_enter/exit` webhook）；§8.6 事件清單同步 |
