...
| Method | Path | 用途 |
|---|---|---|
| `POST` | `/admin/tenants/{tid}/devices/{did}/transfer` | 跨校轉移（改分組 + 觸發 Wipe 保留 PPKG） |
| `POST` | `/admin/tenants/{tid}/devices/{did}/redeploy` | 遠端重新部署（PRD §5.1）：Wipe 保留 PPKG **不改分組**，設備自動回同一 tenant / group |
| `POST` | `/admin/tenants/{tid}/devices/{did}/retire` | 設備退役（doWipe 徹底 + 標記 unenrolled） |
