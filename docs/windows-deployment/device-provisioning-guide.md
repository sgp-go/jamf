...
1. **PPKG（Provisioning Package，預配套件）是初始化的核心載體**：一個 `.ppkg` 檔案可一次完成「建本機帳號 + 連 WiFi + 納管進 MDM」。透過 USB 在開機設定階段（OOBE）套用。
...
curl -X POST \
  ...
  -d '{
    ...
    "localAccounts": [
      { "username": "student", "password": "<學生帳號密碼>", "isAdmin": false },
...
>
> **聯網時機**：enrollment 須連後端。若 PPKG 已配 WiFi，套用後自動連；若沒配，需在 OOBE 的「網路」頁先手動連 WiFi 再繼續。
>
...
- **OOBE 套用需聯網**：enrollment 要連後端，PPKG 建議內含 WiFi。
