...
> 📌 **本文自 `docs/archived/windows-mdm-msix-signing.md` 遷入**——簽名機制與 cert 信任原理與租戶無關，**生產仍適用**。push MSIX 構建見 [push-infrastructure-setup.md](push-infrastructure-setup.md)，Agent MSI 簽名見 [agent-app-build-and-deploy.md](agent-app-build-and-deploy.md)。
>
> ⚠️ 文中「當前實作未包含 / 待加」等表述為撰寫時狀態；設備端信任 cert 的自動下發在多租戶版**已落地**（enrollment hook 自動下發 push cert，見 push-infrastructure §6/§9）。
