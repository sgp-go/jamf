...
| [intune-coexistence-agent-enroll.md](intune-coexistence-agent-enroll.md) | **Intune 共存部署（遙測 only）**：存量 Intune 設備接入遙測——生成 tenant 級共享密鑰 → Intune 派發只帶密鑰的 MSI → Agent 首啟自助換 per-device token；⚠️ 只換遙測，管理面仍歸 Intune |
